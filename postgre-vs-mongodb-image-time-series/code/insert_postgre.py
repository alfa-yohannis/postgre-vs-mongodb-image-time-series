...
from database_setup import recreate_postgres_table
...
def get_table_and_db_sizes(cur, table_name: str) -> dict[str, int]:
    try:
        cur.execute(
            f"""
            SELECT
    ...
    return {
        "table_total_bytes": int(table_total_bytes),
        "table_data_bytes": int(table_data_bytes),
...
def insert_rows(cur, settings, payload) -> tuple[int, float]:
    ...
    while inserted < settings.total_rows:
        ...
        batch.append(
            (
                ...
                psycopg2.Binary(payload.payload_bytes),
        ...
        if len(batch) >= settings.batch_size:
            execute_batch(
                cur,
                f"""
                INSERT INTO {settings.postgres_table_name} (
                    device_id,
                    ts,
                    profile_name,
                    payload_kind,
...
def main() -> None:
    ...
    run_csv_path = settings.result_csv("results_postgres_insert_runs")
    summary_csv_path = settings.result_csv("results_postgres_insert_summary")

    print("PostgreSQL insert benchmark")
    ...
    for run_id in range(1, settings.insert_runs + 1):
        ...
        try:
            with conn.cursor() as cur:
                sizes_before = get_table_and_db_sizes(cur, settings.postgres_table_name)
                rows_inserted, duration = insert_rows(cur, settings, payload)
                conn.commit()
                sizes_after = get_table_and_db_sizes(cur, settings.postgres_table_name)
                rows_in_table_after = get_row_count(cur, settings.postgres_table_name)
        finally:
            conn.close()
        ...
        print(
            ...
            f"table={table_total_after_mb:.2f} MB"
        )
