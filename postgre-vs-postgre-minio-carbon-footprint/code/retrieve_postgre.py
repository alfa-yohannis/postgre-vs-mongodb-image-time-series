...
def main() -> None:
    ...
    run_csv_path = settings.result_csv("results_postgres_retrieve_runs")
    summary_csv_path = settings.result_csv("results_postgres_retrieve_summary")
    ...
    try:
        with conn.cursor() as cur:
            ...
            for _ in range(settings.aggregation_warmup_runs):
                with conn.cursor(name="warmup_retrieve_postgres") as named_cur:
                    named_cur.itersize = 20
                    named_cur.execute(query, (settings.device_id, ts_min, ts_max))
                    for _, payload_data in named_cur:
                        len(payload_data)
            ...
            for run_id in range(1, settings.aggregation_runs + 1):
                ...
                with conn.cursor(name=f"retrieve_postgres_run_{run_id}") as named_cur:
                    named_cur.itersize = 20
                    named_cur.execute(query, (settings.device_id, ts_min, ts_max))
                    byte_sum = 0
                    count = 0
