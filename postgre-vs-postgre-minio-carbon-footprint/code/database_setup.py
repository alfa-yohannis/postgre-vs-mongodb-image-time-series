...
def wait_for_minio(settings: BenchmarkSettings, timeout_sec: int = 90) -> None:
    deadline = time.time() + timeout_sec
    while time.time() < deadline:
        try:
            client = Minio(
            ...
            time.sleep(1)
    raise TimeoutError("Timed out while waiting for MinIO to accept connections.")
...
def open_minio(settings: BenchmarkSettings) -> Minio:
    return Minio(
        settings.minio_endpoint,
        access_key=settings.minio_access_key,
        secret_key=settings.minio_secret_key,
...
def ensure_minio_bucket(settings: BenchmarkSettings) -> None:
    client = open_minio(settings)
    if not client.bucket_exists(settings.minio_bucket):
        client.make_bucket(settings.minio_bucket)


def recreate_minio_bucket(settings: BenchmarkSettings) -> None:
    client = open_minio(settings)
...
def ensure_postgres_minio_schema(settings: BenchmarkSettings) -> None:
    conn = open_postgres(settings)
    try:
        with conn.cursor() as cur:
            cur.execute(
                POSTGRES_MINIO_DDL_TEMPLATE.format(table_name=settings.postgres_minio_table_name)
            )
    finally:
        conn.close()


def recreate_postgres_minio_table(settings: BenchmarkSettings) -> None:
    conn = open_postgres(settings)
    try:
        with conn.cursor() as cur:
            cur.execute(f"DROP TABLE IF EXISTS {settings.postgres_minio_table_name} CASCADE;")
            cur.execute(
                POSTGRES_MINIO_DDL_TEMPLATE.format(table_name=settings.postgres_minio_table_name)
            )
    finally:
