...
from database_setup import recreate_mongo_collection
...
def get_collection_and_db_sizes(db, coll_name: str) -> dict[str, int]:
    ...
    return {
        "table_total_bytes": int(coll_total_bytes),
        "table_data_bytes": int(coll_storage_bytes),
...
def insert_docs(coll, settings, payload) -> tuple[int, float]:
    ...
    while inserted < settings.total_rows:
        ...
        if len(batch) >= settings.batch_size:
            try:
                ...
                print(f"[Mongo Error] batch insert failed: {type(e).__name__}: {str(e)[:100]}...")
            inserted += len(batch)
            batch.clear()
    ...
    if batch:
        try:
            ...
            print(f"[Mongo Error] remainder batch insert failed: {type(e).__name__}: {str(e)[:100]}...")
        inserted += len(batch)
...
def main() -> None:
    ...
    print("MongoDB insert benchmark")
    ...
    if settings.warmup_rows > 0:
        ...
        try:
            ...
            rows_warm, dur_warm = insert_docs(coll, warmup_settings, payload)
    ...
    for run_id in range(1, settings.insert_runs + 1):
        ...
        try:
            ...
            sizes_before = get_collection_and_db_sizes(db, settings.mongo_collection_name)
            rows_inserted, duration = insert_docs(coll, settings, payload)
            sizes_after = get_collection_and_db_sizes(db, settings.mongo_collection_name)
        ...
        print(
            ...
            f"collection={table_total_after_mb:.2f} MB"
        )
