...
\author*[1]{\fnm{Alfa} \sur{Yohannis}}\email{alfa.ryano@gmail.com}
...
Every combination of architecture, operation, and resolution runs inside its own CodeCarbon~\citep{codecarbon} energy tracker, so per-resolution energy and emissions are measured \emph{directly}, rather than estimated by spreading one coarse measurement across resolutions as in our earlier work. CodeCarbon reads processor and memory power via Intel RAPL and converts the total energy to CO\textsubscript{2}eq using the Indonesian grid intensity; we report the duration, total energy, and emissions of insertion, bulk retrieval, and latest-frame reads at every resolution. Reported figures are means over five repeated runs, accompanied by standard deviations as a measure of run-to-run variability. Because emissions are simply energy multiplied by the grid intensity, the \emph{ranking} of the architectures depends on energy alone and is therefore the same in every region (Section~\ref{subsec:grid}).

\subsection{Failover and the unstorable cell}\label{subsec:failover}
To keep the experiment robust and the data trustworthy, each measurement is held in memory and saved only after it completes successfully; a measurement that fails is retried, and after repeated failures it is recorded as \emph{skipped} (with the architecture, resolution, number of attempts, and error) and the sweep continues to the next case. Exactly one case triggered this path: \textbf{MongoDB at 6K} (Section~\ref{subsec:bson}). The benchmark therefore reports a clean three-way comparison from 360p to 5K and a two-way PostgreSQL-versus-hybrid comparison at 6K, with the omission stated explicitly rather than silently mishandled.

%%=====================================================================%%
\section{Results}\label{sec:results}
%%=====================================================================%%

We report insertion throughput and storage amplification (Section~\ref{subsec:insert}), bulk retrieval and latest-frame reads (Section~\ref{subsec:retrieval}), the central per-resolution carbon comparison with its SCI and embodied-carbon implications (Section~\ref{subsec:carbon}), multi-region grid sensitivity (Section~\ref{subsec:grid}), and MongoDB's 16~MB ceiling (Section~\ref{subsec:bson}). All values are directly measured on identical hardware and CodeCarbon configuration; standard deviations across five runs are small (Tables~\ref{tab:write}--\ref{tab:read}), supporting the comparison; the per-operation winners are summarised in the decision framework (Section~\ref{sec:framework}).

\subsection{Insertion throughput and storage amplification}\label{subsec:insert}
Insertion (Figure~\ref{fig:insert}, Table~\ref{tab:write}) exhibits a clear, monotone reordering with payload size. At low resolution MongoDB ingests dramatically faster than the alternatives---4{,}194~rows/s at 360p, over $6\times$ PostgreSQL and $56\times$ the hybrid---because each frame compresses to a near-empty bucket (storage amplification $0.001\times$) that WiredTiger writes entirely in cache. PostgreSQL is the middle ingester at low resolution (batched inserts avoid a per-object network call), while PostMin pays the fixed cost of one MinIO PUT per payload and is slowest. As payloads grow the order inverts: MongoDB falls below PostgreSQL between 1440p and 4K and becomes the slowest engine at 4K--5K as WiredTiger serialises multi-megabyte BSON documents through B-tree pages~\citep{wiredtiger_btree_architecture} and compression fails; PostMin, whose throughput is nearly flat, overtakes PostgreSQL at 4K and remains highest through 6K. Storage amplification (Figure~\ref{fig:storage}) explains much of the downstream carbon: Postgre holds a stable $\sim$1.04$\times$ (TOAST out-of-line layout) and PostMin $\sim$1.00$\times$ (raw object size) at all resolutions, whereas MongoDB undergoes a regime change---$0.10\times$ at 1080p (Snappy crushes small documents dominated by metadata) rising to $1.59\times$ at 4K and $3.00\times$ at 5K as high-entropy JPEG defeats block compression.
...
Res. & Postgre & MongoDB & PostMin & Postgre & Mongo & PostMin \\
\midrule
...
\begin{itemize}
  \item \textbf{Standard resolutions (360p--1440p): MongoDB is greenest}, and dramatically so at the low end---40~mg at 360p versus 191~mg (Postgre) and 933~mg (PostMin), a $4.8$--$23\times$ advantage---because near-empty compressed buckets ingest extremely fast. PostMin is the \emph{worst} here, penalised by a per-payload HTTP PUT on small images.
...
\end{itemize}
Insertion dominates the footprint (Figure~\ref{fig:breakdown}); bulk retrieval is the second contributor and latest-frame reads are negligible ($<5$~mg). Cumulatively over the common 360p--5K range, the hybrid is the lowest-carbon architecture at 15.6~g~CO\textsubscript{2}eq, $24.7\%$ below inline PostgreSQL (20.8~g) and $25.9\%$ below MongoDB (21.1~g): despite winning every standard resolution, MongoDB ends highest cumulatively because the high-resolution profiles dominate total energy. A CPU/RAM decomposition shows CPU accounts for $\approx$53\% of energy for the inline engines (Postgre, MongoDB) but only $\approx$41\% for the hybrid, consistent with PostMin offloading payload handling away from the database CPU path.
...
\begin{table}[t]
\caption{Directly measured per-resolution carbon (mg~CO\textsubscript{2}eq; insertion + bulk retrieval + latest-frame read). Lower is better; \textbf{bold} = greenest at that resolution.}\label{tab:carbon}
\small
\begin{tabular}{@{}lrrrl@{}}
\toprule
Res. & Postgre (BYTEA) & MongoDB & PostMin (Postgre+MinIO) & Greenest \\
\midrule
360p  & 191.1   & \textbf{40.1}   & 933.2   & Mongo \\
480p  & 293.7   & \textbf{74.2}   & 1490.8  & Mongo \\
720p  & 667.9   & \textbf{194.3}  & 1587.9  & Mongo \\
1080p & 1507.3  & \textbf{457.3}  & 1852.5  & Mongo \\
1440p & 2592.3  & \textbf{1048.6} & 2035.0  & Mongo \\
4K    & 5639.9  & 5739.5 & \textbf{2628.1} & PostMin \\
5K    & 9866.6  & 12564.4 & \textbf{5113.6} & PostMin \\
6K    & 13908.6 & --     & \textbf{6072.0} & PostMin \\
\midrule
\multicolumn{1}{@{}l}{Cum.\ 360p--5K} & 20758.9 & 21118.4 & \textbf{15641.2} & PostMin \\
\botrule
\end{tabular}
...
\textbf{Carbon of the read operations.} The same crossover recurs within each read operation (Table~\ref{tab:dimcarbon}). For bulk retrieval, MongoDB is greenest up to 720p and the hybrid from 1080p---an earlier crossover than insertion, because the hybrid's HTTP streaming avoids pulling large values through the database engine. Latest-frame reads cost under 5~mg throughout and are led by the hybrid wherever the workload is non-trivial. Because insertion (Table~\ref{tab:sci}) is by far the largest contributor, these per-operation winners carry through to the totals in Table~\ref{tab:carbon}.

\begin{table}[t]
\caption{Carbon of the two read operations (mg~CO\textsubscript{2}eq per measured run set), completing the per-operation breakdown alongside insertion (Table~\ref{tab:sci}) and the total (Table~\ref{tab:carbon}). Lower is better; \textbf{bold} = greenest per row.}\label{tab:dimcarbon}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
...
Res. & Postgre & Mongo & PostMin & Postgre & Mongo & PostMin \\
\midrule
360p  & 55.1   & \textbf{12.9}  & 97.0  & 0.2 & \textbf{0.1} & 0.2 \\
480p  & 93.8   & \textbf{29.8}  & 99.2  & 0.3 & \textbf{0.2} & 0.3 \\
720p  & 197.5  & \textbf{70.8}  & 113.8 & 0.3 & \textbf{0.2} & 0.3 \\
1080p & 472.5  & 169.0 & \textbf{146.1} & 0.4 & 0.4 & \textbf{0.3} \\
1440p & 752.0  & 294.5 & \textbf{194.6} & 0.6 & 0.5 & \textbf{0.3} \\
4K    & 1645.9 & 676.1 & \textbf{308.1} & 1.2 & 1.6 & \textbf{0.4} \\
5K    & 3003.5 & 1892.4 & \textbf{446.9} & 2.0 & 4.4 & \textbf{0.5} \\
6K    & 4272.2 & --    & \textbf{645.3} & 2.6 & --  & \textbf{0.6} \\
\botrule
\end{tabular}
\footnotetext{Latest-frame carbon is negligible in absolute terms ($<5$~mg); the hybrid leads it from 1080p upward, with MongoDB marginally lower only where all three are near zero.}
\end{table}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Grid region & g/kWh & Postgre & MongoDB & PostMin \\
\midrule
...
\midrule
$\leq$1440p, ingest-heavy & Mongo & Greenest by $5$--$23\times$; near-zero storage amplification; fastest completion dominates carbon. \\
$\geq$4K, any emphasis & PostMin & $\sim$half the carbon of inline storage; flat storage; HTTP streaming makes reads cheap. \\
Mixed / uncertain resolution & Postgre & Robust pivot: never worst, single transactional boundary, operates over the full range. \\
...
To make the emissions actionable without assuming a particular installation, we express them as a measured \emph{per-frame} rate that a reader can scale to any deployment. At 4K, storing one frame costs a directly measured $1.16$~mg~CO\textsubscript{2}eq with the hybrid, $2.00$~mg with inline PostgreSQL, and $2.53$~mg with MongoDB. These per-frame rates are obtained directly from the measurements: each is the 4K insertion emission divided by the $2{,}000$ frames inserted per run---equivalently, the Software Carbon Intensity reported in Table~\ref{tab:sci} (mg~CO\textsubscript{2}eq per $1{,}000$ frames) divided by $1{,}000$. The hybrid's advantage over inline PostgreSQL is therefore $0.84$~mg per stored frame. Treating the measured per-frame rate as constant, an operator obtains a first-order annual footprint by multiplying it by their camera count, capture rate, and retention period (and by a data-centre Power Usage Effectiveness factor, $\approx$1.5~\citep{shehabi_us_datacenter}, for the facility total); at very large scale the rate may drift with batching, contention, and database growth, so this is an estimate rather than a guarantee. As a single normaliser to fix scale, take $1{,}000$ cameras each capturing one frame per minute for a year: that is $1{,}000\times60\times24\times365=5.26\times10^{8}$ stored frames. The annual carbon is then the per-frame rate times this frame count, converting milligrams to kilograms---for the hybrid, $1.16~\mathrm{mg}\times(5.26\times10^{8})/10^{6}\approx610$~kg~CO\textsubscript{2}eq. The three architectures' annual footprints are summarised in Table~\ref{tab:fleet}. Concretely, in a case where an organisation decides to migrate its 4K image tier from inline PostgreSQL (Postgre) to the hybrid (PostMin), it would reduce its ingestion carbon footprint by $\approx$440~kg~CO\textsubscript{2}eq per year for every $1{,}000$ cameras (at one frame per minute)---a reduction comparable to driving an average passenger car about $2{,}600$~km (roughly the road distance from New York City to Houston, at a representative $\approx$0.17~kg~CO\textsubscript{2}eq per vehicle-km~\citep{desnz_ghg_factors}), or to the yearly CO\textsubscript{2} uptake of about $21$ mature trees (taking $\approx$21~kg~CO\textsubscript{2} per tree per year~\citep{nowak_urban_trees}). This normaliser is illustrative, not a claim about a specific site; the underlying quantity is the measured per-frame rate.
...
Architecture & Per frame & Per normaliser & With PUE 1.5 \\
             & (mg) & (kg/yr) & (kg/yr) \\
\midrule
PostMin (hybrid) & \textbf{1.16} & \textbf{610}   & \textbf{914}   \\
Postgre (BYTEA)  & 2.00          & 1{,}049        & 1{,}574        \\
Mongo            & 2.53          & 1{,}330        & 1{,}995        \\
\midrule
Saving (Postgre $\rightarrow$ PostMin) & 0.84 & 440 & 660 \\
\botrule
\end{tabular}
\footnotetext{Per-frame rate $=$ directly measured 4K insertion carbon $\div$ 2{,}000. Annual $=$ per-frame rate (mg) $\times$ cameras $\times$ frames per camera per year $/\,10^{6}$ (at 1 frame/min, $60\times24\times365=525{,}600$ frames/yr); computed from unrounded rates. The $\approx$440~kg/yr saving $\approx$ $2{,}600$~km of car travel $\approx$ annual CO\textsubscript{2} uptake of $21$ mature trees. Multiply linearly for other fleets.}
\end{table}
...
\textbf{Why the crossover occurs.} The three engines occupy different points on a single trade-off between fixed per-operation overhead and per-byte payload handling. MongoDB's inline BSON layout is extraordinarily efficient while documents stay small and cache-resident: buckets compress to near-nothing and ingest in cache. But WiredTiger keeps documents in their \emph{uncompressed} form in working memory and writes the entire BSON document---including the multi-megabyte image---through its on-disk index pages~\citep{wiredtiger_btree_architecture}; as payloads grow, Snappy~\citep{google_snappy} fails on high-entropy JPEG, storage amplification climbs to $3.00\times$, and at the largest sizes the memory cache fills, forcing the system to evict data to disk and stalling the very operations that are trying to write~\citep{percona_wiredtiger_eviction}. PostgreSQL's TOAST~\citep{postgresql_toast_docs} relocates images out-of-line, keeping the main index compact and the write path roughly independent of resolution ($1.04\times$), which yields stable, middle-of-the-road carbon. The hybrid removes the image from the relational database entirely, so PostgreSQL records only metadata---its write-ahead log (WAL, the change journal a database writes before applying updates, used for crash recovery) and indexes grow with the metadata rather than the image size---while MinIO streams the object; this is the most energy-efficient path once payloads are large, at the cost of a fixed HTTP round-trip that makes it the worst choice for small images.
...
\textbf{Threats to validity.} Results are from a single host and container topology; distributed deployments may differ, though the relative comparison holds because all engines ran under identical conditions. Per-resolution emissions are now directly measured (removing the estimation assumption of our earlier work), but energy accuracy depends on the Intel RAPL counters; on computers where RAPL is unavailable, CodeCarbon falls back to Thermal Design Power (TDP) estimates---nameplate power ratings rather than live measurement. We benchmark a single client issuing requests sequentially; multi-client concurrency is effectively a \emph{different} study, because it changes how energy is attributed and introduces contention noise. Single-client sequential access is, by contrast, a clean and reproducible baseline, and since the emission crossover is driven by payload size---a structural property of each storage design---rather than by client count, we expect concurrency to shift the \emph{absolute} figures but not the architectural ordering; a multi-client evaluation is therefore a reasonable item to leave for future work. Likewise, the batch size is fixed at~50: larger batches mainly reduce per-round-trip overhead for the inline engines (Postgre and Mongo) but not the one-object-per-PUT cost of the hybrid, so the crossover is expected to be robust to batching, and a batch-size sweep is a natural extension. Payloads are synthetic quality-90 JPEGs derived from one source asset; because quality-90 JPEG retains the high visual entropy of real photographs, the defeated-compression and storage-amplification behaviour should carry over, although real feeds may vary in entropy and compressibility, and validating on frames extracted from real high-resolution video is future work. Finally, the embodied-carbon and fleet figures are first-order estimates using representative published factors and stated assumptions, intended to convey magnitude rather than site-specific precision.
