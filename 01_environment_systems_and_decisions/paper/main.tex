...
Where that transition falls is a property of the payload rather than of the machine, and a controlled comparison separates the two (Table~\ref{tab:costmodel}). Holding the payload construction fixed while changing the host, from an eight-core i9 with 31~GB to a four-core i7 with 15~GB, changes every absolute figure but leaves the ordering intact, with MongoDB lowest at every payload size measured on both machines. Holding the host fixed while changing the construction introduces a transition that was not present before. The payload therefore governs whether a boundary falls within a given range, while the host, over the range measured on both machines, does not reorder the architectures.
