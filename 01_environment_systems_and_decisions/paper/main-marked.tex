...
\citet{diva_timescaledb_mongodb_eval} & numeric sensor & $\sim$KB & Timescale, Mongo & No & Reports MongoDB ingesting faster than TimescaleDB. We reproduce this at low resolution but show it is \emph{resolution-dependent}, not a property of the engines. \\
\textbf{This work} & \textbf{JPEG images} & \textbf{7.1~MB (6K)} & \textbf{Postgre, Mongo, PostMin} & \textbf{Yes (RAPL)} & \textbf{Directly measured per-operation carbon, a resolution-dependent emission crossover, a two-parameter cost model that transfers the rule to other hardware, and an infeasibility result at 6K.} \\
\botrule
\end{tabular}
\end{table}
...
\botrule
\end{tabular}
\footnotetext{{\color{markup}$^{\dagger}$The real-frame validation of Section~\ref{subsec:realframes} ran on a \emph{different} computer, with a four-core Core i7-1165G7 and 15.30~GB of memory; see Table~\ref{tab:env2}. The architectural ordering is unchanged between the two.}}
\end{table}
...
\begin{table}[htbp]
\color{markup}
\caption{The operational deployment used for validation. Frames were captured simultaneously from all three cameras for ten minutes at 5~frames per second. ``Payload'' is the mean encoded size at the camera's native resolution.}\label{tab:deployment}
\small
\begin{tabular}{@{}llrrr@{}}
\toprule
...
\botrule
\end{tabular}
\footnotetext{All 8{,}724 frames were verified byte-distinct, and every frame was checked automatically for the presence of a face before use; none was found. Capture rate was chosen as 5~frames per second because face capture is reliable at that rate, and a higher rate would multiply stored frames and their carbon without improving the recognition the deployment performs.}
\end{table}
...
$\gtrsim$7~MB per frame ($\gtrsim$6K)$^\dagger$ & Postgre or PostMin & MongoDB \emph{infeasible}: bucketed documents exceed the 16~MB BSON limit. \\
\botrule
\end{tabular}
...
Where that transition falls is a property of the payload rather than of the machine, and a controlled comparison separates the two (Table~\ref{tab:costmodel}). Holding the payload construction fixed while changing the host, from an eight-core i9 with 31~GB to a four-core i7 with 15~GB, changes every absolute figure but leaves the ordering intact, with MongoDB lowest at every size measured. Holding the host fixed while changing the construction introduces a transition that was not there before. The boundary moves with the data, not the hardware.

\begin{table}[htbp]
\color{markup}
\caption{Isolating what moves the crossover. Each pair changes one factor. Holding the payload construction fixed and changing the host leaves the ordering unchanged; holding the host fixed and changing the construction introduces a crossover. The transition is therefore a property of the payload, not of the machine.}\label{tab:costmodel}
\small
\begin{tabular}{@{}llll@{}}
\toprule
Comparison & Varied & Held fixed & Crossover in $0.03$--$0.80$~MB \\
\midrule
i9 vs i7, collage & host & payload & none in either; Mongo lowest throughout \\
collage vs real, i7 & payload & host & none $\rightarrow$ Mongo\,$\rightarrow$\,Postgre at $0.09$--$0.15$~MB \\
\botrule
\end{tabular}
\footnotetext{Brackets are bounded by adjacent measurement points; no interpolation is implied. The collage reaches $0.80$~MB in this range and shows no transition, whereas real frames cross an order of magnitude earlier in payload size.}
\end{table}
...
\textbf{Ethics approval.} This study involved no human participants and no personal data. The real-frame corpus of Section~\ref{subsec:realframes} was captured by an author, in a space with no other person present, with no biometric identifier recorded or retained; every saved frame was verified automatically to contain no detectable face before use. The images serve only as representative binary payloads for storage measurement, and no identification, recognition, or tracking was performed on them.}
