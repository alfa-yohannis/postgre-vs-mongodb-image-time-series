...
\textbf{This work} & \textbf{JPEG images} & \textbf{7.1~MB (6K)} & \textbf{Postgre, Mongo, PostMin} & \textbf{Yes (RAPL)} & \textbf{Directly measured per-operation carbon, a payload-dependent emission crossover, a two-parameter cost model that transfers the rule to other hardware, and an infeasibility result at 6K.} \\
\botrule
\end{tabular}
\end{table}
...
Where that transition falls is a property of the payload rather than of the machine, and a controlled comparison separates the two (Table~\ref{tab:costmodel}). Holding the payload construction fixed while changing the host, from an eight-core i9 with 31~GB to a four-core i7 with 15~GB, changes every absolute figure but leaves the ordering intact, with MongoDB lowest at every payload size measured on both machines. Holding the host fixed while changing the construction introduces a transition that was not there before. The boundary moves with the data, not the hardware.
...
\midrule
i9 vs i7, collage & host & payload & none in either over the common range \\
