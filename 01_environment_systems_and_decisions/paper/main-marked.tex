...
% Float placement: with 15 tables and 7 figures the defaults push floats to the
...
\begin{tabular}{@{}p{0.17\textwidth}p{0.13\textwidth}p{0.07\textwidth}p{0.13\textwidth}p{0.08\textwidth}p{0.28\textwidth}@{}}
\toprule
...
CrateDB benchmark~\citep{cratedb_ts_benchmark} & numeric IoT metrics & $\sim$KB & CrateDB et al. & No & Optimises for query latency on numeric metrics; storage volume is incidental. We find storage amplification varies by three orders of magnitude across engines and drives both operational and embodied carbon. \\
\citet{diva_timescaledb_mongodb_eval} & numeric sensor & $\sim$KB & Timescale, Mongo & No & Reports MongoDB ingesting faster than TimescaleDB. We reproduce this at small payloads but show it is \emph{payload-dependent}, not a property of the engines. \\
\textbf{This work} & \textbf{JPEG images} & \textbf{7.1~MB (6K)} & \textbf{Postgre, Mongo, PostMin} & \textbf{Yes (RAPL)} & \textbf{Directly measured per-operation carbon, a payload-dependent emission crossover, a measurement procedure that transfers the rule to other installations, and an infeasibility result at 6K.} \\
\botrule
\end{tabular}
\end{table}
...
Table~\ref{tab:carbon} and Figure~\ref{fig:carbon} report directly measured per-resolution emissions and constitute the central result. They reveal a \emph{payload-dependent emission crossover with no universal winner}, in which MongoDB and the hybrid are mirror images and inline PostgreSQL is the pivot:
\begin{itemize}
...
\textbf{The capture application.} Because the corpus must be reproducible by others, the recorder is released with the artifact and specified here. It is a small application, of the kind an operator of such a deployment would actually run, and its design choices matter to the measurements it produces.
...
\begin{table}[htbp]
\color{markup}
\caption{The face-recognition-based attendance system used as the real-case deployment. Frames were captured simultaneously from all three cameras for ten minutes at 5~frames per second. ``Payload'' is the mean encoded size at the camera's native resolution.}\label{tab:deployment}
\small
\begin{tabular}{@{}llrrr@{}}
\toprule
Camera & Transport & Native resolution & Frames & Payload (KB) \\
\midrule
Networked (IP) & H.264 / RTSP     & $1920\times1080$ & 2{,}845 & 164 \\
USB camera 1   & Motion JPEG      & $1920\times1080$ & 2{,}988 & 157 \\
USB camera 2   & Motion JPEG      & $1280\times720$  & 2{,}891 &  69 \\
\botrule
\end{tabular}
...
Three findings follow (Table~\ref{tab:realframes} and Figure~\ref{fig:realframes}). First, \textbf{the architectural conclusion holds}: MongoDB is the lowest-carbon architecture at every resolution from 360p to 1080p, exactly as the collage sweep reports for this range, and the result replicates independently on a second camera. Second, \textbf{real payloads are far smaller than the single-image collage}, by $3.1\times$ at 360p rising to $5.4\times$ at 1080p, confirming that the quality-90 collage is close to a worst case for compressibility rather than a typical one. Third, and most consequential, \textbf{storing one repeated image rather than distinct ones materially overstates MongoDB's advantage}, and the effect can be isolated. Comparing at matched payload size on the same host (Table~\ref{tab:repetition}), PostgreSQL and the hybrid are essentially unaffected by repetition, becoming slightly cheaper with real frames, whereas MongoDB's ingestion carbon rises by $2.4$ to $2.9\times$ and its storage amplification by $16$ to $25\times$. The asymmetry has a single cause: MongoDB is the only one of the three that compresses \emph{across} rows, bucketing many frames into one document in which Snappy removes repeated content almost entirely. TOAST compresses per value and MinIO stores per object, so neither benefits from repetition and neither is distorted by it.
...
\botrule
\end{tabular}
\footnotetext{MongoDB is the greenest architecture in every row of both runs, and on the second camera as well. Storage amplification for PostgreSQL ($1.03$--$1.09\times$) and the hybrid ($1.00$--$1.02\times$) is unchanged by payload realism, as expected: neither compresses across rows.}
\end{table}

The real-case runs were executed on a \emph{different computer} from the controlled benchmark (Table~\ref{tab:env2}), so they vary hardware as well as payload. The validation host is an ultraportable-class Intel Core i7-1165G7 with four physical cores, against the eight-core mobile-workstation Core i9-11900H used for the controlled benchmark: half the cores, half the memory, and a proportionally scaled MongoDB cache of 14~GB in place of 30~GB. Absolute figures differ between the two machines, as they must. The architectural ordering does not. Because the crossover is driven by the ratio of fixed to per-byte cost rather than by absolute speed, halving the core count and the cache rescales both terms without reordering them, which is what the mechanism of Section~\ref{subsec:model} implies and what we observe.}

\begin{table}[htbp]
\color{markup}
\caption{The second environment used for the real-frame validation, beside the environment of the controlled benchmark. The two differ in processor class, core count, and memory, so agreement between them is evidence of hardware independence rather than a repeated measurement.}\label{tab:env2}
\small
\begin{tabular}{@{}lll@{}}
\toprule
...
The measurements translate into operational guidance for sustainable building-monitoring infrastructure. The decision variable is the \emph{payload a camera actually produces}; the secondary variable is workload emphasis (write-heavy ingestion, read-heavy analytics, or storage-bound archival). {\color{markup}Resolution is the convenient label for payload size but not a reliable substitute for it, for two reasons this study can quantify. Cameras of the same nominal resolution differ by up to 12\% in encoded size (Section~\ref{subsec:realframes}), and, more importantly, payload \emph{content} moves the crossover itself: the ingestion crossover falls between $1.40$ and $2.99$~MB for the high-entropy collage payload but between $0.09$ and $0.15$~MB for real camera output, because bucketed identical rows compress in a way real frames do not. An operator keying the decision to resolution alone, using boundaries calibrated on worst-case payloads, would therefore select MongoDB for a deployment where inline PostgreSQL is greener, which is exactly the situation Section~\ref{subsec:applied} encounters.
...
\toprule
Operating regime (payload per frame) & Choice & Rationale \\
\midrule
...
$\gtrsim$7~MB per frame ($\gtrsim$6K)$^\dagger$ & Postgre or PostMin & MongoDB \emph{infeasible} on any hardware: bucketed documents exceed the 16~MB BSON limit. \\
\botrule
\end{tabular}
\footnotetext{$^\dagger$A single 6K frame ($\approx$7~MB) is itself well within MongoDB's 16~MB per-document limit; the limit is breached not by one payload but because MongoDB's native time-series \emph{bucketing} groups several multi-megabyte frames into one internal bucket document, whose combined size exceeds 16~MB (see Section~\ref{subsec:bson}). This row is a protocol constraint and holds on any hardware.}
\end{table}

{\color{markup}\subsection{Why the crossover exists, and how to locate it elsewhere}\label{subsec:model}
...
Where that transition falls depends on both the payload and the host, and a controlled comparison separates the two contributions (Table~\ref{tab:costmodel}). Holding the host fixed while changing the payload construction introduces a transition that was not present before, at an order of magnitude smaller payload. Holding the payload fixed while changing the host moves the transition too, and in the direction the mechanism predicts: on the smaller machine MongoDB's advantage is exhausted sooner. At 1440p, where the payload is $1.40$~MB, MongoDB emits $0.41\times$ the hybrid's ingestion carbon on the larger host but $1.01\times$ on the smaller one. Its advantage at that payload has disappeared entirely.

The reason is the same mechanism that governs the crossover itself. WiredTiger holds documents uncompressed in cache, so MongoDB's live working set scales with the payload: one insertion run of 2{,}000 frames at 4K is $5.8$~GB of poorly compressible data. On a host with 31~GB that is comfortably absorbed. On one with 15.30~GB it is not, and the consequence is not merely slower but categorical, as Section~\ref{subsec:memory} reports.

\begin{table}[htbp]
\color{markup}
\caption{Isolating what moves the crossover. Each row changes one factor while holding the other fixed. Both matter: the payload determines whether a transition falls within a given range, and the host determines where. Brackets are bounded by adjacent measurement points, and no interpolation is implied.}\label{tab:costmodel}
\small
\begin{tabular}{@{}lllp{0.36\textwidth}@{}}
\toprule
Comparison & Varied & Held fixed & Effect on the transition \\
\midrule
collage vs real frames, one host & payload & host & introduces a Mongo\,$\rightarrow$\,Postgre transition at $0.09$--$0.15$~MB where the collage shows none below $0.80$~MB \\
i9 vs i7, collage payload & host & payload & MongoDB's margin over the hybrid at $1.40$~MB falls from $0.41\times$ to $1.01\times$, so the transition moves to a smaller payload on the smaller host \\
\botrule
\end{tabular}
\footnotetext{The second row states a change in margin rather than a bracket. Run-to-run variability on the smaller host is substantially higher (coefficient of variation up to $28\%$, against under $6\%$ on the larger host), a symptom of the memory pressure described in Section~\ref{subsec:memory}, so the exact position of its transition cannot be resolved. The direction and magnitude of the shift are larger than that variability and are not in doubt.}
\end{table}

\begin{table}[htbp]
\color{markup}
\caption{The effect of storing one repeated image rather than distinct ones, at matched payload size on the same host. Values are ingestion carbon in mg~CO\textsubscript{2}eq per frame, with MongoDB's storage amplification beneath. Only MongoDB is affected, and it is affected in both directions at once: it costs more and stores more.}\label{tab:repetition}
\small
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
...
{\color{markup}\subsection{Applying the framework to the deployment}\label{subsec:applied}
The normaliser above is illustrative. We therefore close by applying the framework to the installation that supplied the corpus of Section~\ref{subsec:realframes}, using its measured payload rather than an assumed one. Figures are given per camera, so that a site of any size is obtained by multiplication.
...
\caption{The same measurement for the real-case corpus, over the payload range the deployment actually produces. The upper axis shows where this camera's resolutions fall on the payload axis; the relation between the two is specific to this camera, since another sensor at the same nominal resolutions would place the ticks elsewhere. The crossover bracket lies between $0.09$ and $0.15$~MB, an order of magnitude below the collage bracket of Figure~\ref{fig:crossover_collage}, and the deployment operates at its upper edge. A controlled comparison (Table~\ref{tab:costmodel}) attributes this gap to the payload rather than to the host: repeating the collage measurement on the second machine leaves the ordering unchanged, whereas changing the payload construction on one machine introduces the transition. The position of the boundary is therefore a property of what the cameras produce, not of the hardware that stores it.}\label{fig:applied}
\end{figure}
...
\textbf{Use of AI-assisted technologies.} Generative AI tools were used solely for language editing, that is improving the grammar, clarity, and phrasing of text written by the authors. No AI tool was used to generate, analyse, or interpret the experimental data, to produce the figures or tables, or to draw the scientific conclusions reported here. The authors reviewed and edited all such output and take full responsibility for the entire content of this manuscript. AI is not credited as an author, in accordance with the journal's editorial policy.

\textbf{Data and code availability.} The benchmark harness, Docker configurations, raw per-run measurement CSVs, and figure-generation scripts are provided as a reproducibility artifact accompanying this submission, and will be deposited in a public repository with a persistent identifier on acceptance. The collage source image is redistributable under its original licence. The corpus of real camera frames used in Section~\ref{subsec:realframes} is released with the artifact; it was recorded specifically for this study, contains no identifiable person, and the recording tool used to produce it is included so that the corpus can be regenerated in another setting.

\textbf{Ethics approval.} This study involved no human participants and no personal data. Although the deployment described in Section~\ref{subsec:realframes} performs face recognition in normal operation, no recognition function was used here and no enrolled identity was accessed: the cameras were read directly for the purpose of this study. The corpus was captured by an author, in a space with no other person present, with no biometric identifier recorded or retained, and every saved frame was verified automatically to contain no detectable face before use. The images serve only as representative binary payloads for storage measurement, and no identification, recognition, or tracking was performed on them.}
