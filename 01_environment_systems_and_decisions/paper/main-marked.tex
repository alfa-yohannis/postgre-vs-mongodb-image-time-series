...
{\color{markup}\subsection{The same payloads on two machines}\label{subsec:twohosts}
The measurements above come from one computer. Repeating the entire collage sweep on a second, markedly smaller machine tests whether the ordering and its crossover are properties of the architectures or of the host. The payload sizes are identical in both columns of Table~\ref{tab:twohosts}, as is the software, so the two differ only in the machine.

Three things follow. The \textbf{ordering at small payloads is unchanged}: MongoDB is greenest at every payload up to $0.80$~MB on both machines, so the low-payload conclusion does not depend on the host. The \textbf{crossover moves}, however, and in the direction the mechanism predicts. On the larger machine the hybrid overtakes MongoDB between $1.40$ and $2.99$~MB; on the smaller one it has already overtaken it by $1.40$~MB. The hybrid's advantage is that it keeps bytes out of the database write path, and a smaller host is precisely where that path is most constrained, so the benefit arrives at a smaller payload. At $1.40$~MB MongoDB emits $0.41\times$ the hybrid's carbon on the larger machine and $1.01\times$ on the smaller: its advantage there has gone.
...
\begin{table}[htbp]
\color{markup}
\caption{The same collage payloads measured on two machines. Values are ingestion carbon in mg~CO\textsubscript{2}eq per frame; \textbf{bold} marks the lowest in each row. Payload sizes, software versions, and protocol are identical across the two column groups, so any difference is attributable to the host.}\label{tab:twohosts}
\small
\setlength{\tabcolsep}{4pt}
...
\midrule
1440p & 1.40 & 0.1840 & 0.1840 & \textbf{0.0754} & 0.1889 & \textbf{0.1274} & 0.1287 \\
...
{\color{markup}\subsection{Payload size, not resolution, determines the outcome}\label{subsec:sizevsres}
...
Two regularities across the four operations motivate a single decision rule. First, in every operation the lowest-carbon architecture is also the fastest, at every payload measured. This is not a coincidence: emissions are proportional to energy, energy is power integrated over time, and the architecture that finishes an operation soonest therefore almost always emits least doing it. The carbon question and the performance question share an answer, which is convenient for a practitioner who need not trade one against the other.
...
{\color{markup}We do not tabulate these crossovers by resolution. As Section~\ref{subsec:sizevsres} shows, the resolution at which any of them falls is a property of the camera and the corpus rather than of the architectures, so a table of resolution boundaries would encode our payloads rather than the reader's.}

\subsection{Selecting an architecture by operating regime}\label{subsec:selection}
The measurements translate into operational guidance for sustainable building-monitoring infrastructure. {\color{markup}The decision variable is the payload a camera actually produces, for the reasons established in Section~\ref{subsec:sizevsres}; the secondary variable is workload emphasis (write-heavy ingestion, read-heavy analytics, or storage-bound archival). Resolution enters only as an indicative guide, since two cameras sold at the same resolution can sit at different points on the payload axis and a rule calibrated on one corpus mis-selects for another.}
...
The framework is stated in payload sizes, so it can be applied to any installation whose payload is known. We close by applying it to the deployment that supplied the corpus of Section~\ref{subsec:realframes}, using its measured payload rather than an assumed one, and expressing the result as a measured per-frame rate that a reader can scale to any site. Figures are given per camera, so that a site of any size follows by multiplication.
