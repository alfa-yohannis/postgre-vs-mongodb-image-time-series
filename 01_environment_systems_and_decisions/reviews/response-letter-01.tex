...
The submitted benchmark inserted one collage image for every row, so all 2{,}000 stored rows were
byte-identical, which is reproducible but not what a camera produces. We therefore drew a corpus of real
frames from a \textbf{face-recognition-based attendance system} in operational use: an installation that
captures images from one networked camera and two USB cameras in order to identify people at entry. This
is precisely the class of deployment the paper addresses, in that it retains image streams alongside
timestamped metadata in a continuously running data tier. We take from it only the \emph{storage}
workload, namely the images its cameras produce at the resolutions and capture rate the installation
uses. The recognition pipeline itself, its detection, embedding and matching stages and their accuracy
and energy cost, remains outside the scope of the paper, which concerns the carbon cost of retaining
images rather than of analysing them; we have made that boundary explicit in the manuscript so that
the reported figures are not read as a whole-system footprint. Ten minutes of capture at 5 frames per second
yielded 2{,}845--2{,}988 frames per camera; every frame was verified byte-distinct, and the benchmark was
re-run with a different real image for every inserted row. Three results follow:
